...
def test_macos_launcher_is_loopback_only_and_uses_external_state():
    ...
    assert 'export AGENT_CHAT_DB="${COMMUNITY_DATA_DIR}/neuroblend_v5_15.sqlite3"' in launcher
...
def test_installer_requires_release_marker_and_uses_recoverable_updates():
    ...
    assert "/usr/bin/xattr -dr com.apple.quarantine \"${PERPLEXITY_HELPER_APP}\"" in installer
    assert "Never clear quarantine broadly" in installer
