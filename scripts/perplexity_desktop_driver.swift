...
private func folderChooserSurface(
    ...
    // not scan arbitrary panels: require LaunchServices/ViewBridge to resolve
    // the panel's ultimate host back to this exact Perplexity process, then
    // require the strict open-panel accessibility signature before using it.
    for panel in NSRunningApplication.runningApplications(
        withBundleIdentifier: openPanelBundleIdentifier
    ) where panelBelongsToPerplexity(panel, perplexityApp: perplexityApp) {
...
private func selectWorkspaceFolder(
    ...
    let chipCountBefore = workspaceFolderChipCount(root, folder: folder)
    ...
    defer {
        if !selectionConfirmed {
            if let activeChooser = folderChooserSurface(perplexityApp: app) {
                ...
                // If the native panel already accepted but the expected chip
                // never appeared, discard this unsent draft and its request-
                // scoped folder grant.
    ...
    let chipAttached = chooserClosed && waitUntil(timeout: 5, interval: 0.2) {
        workspaceFolderChipCount(root, folder: folder) > chipCountBefore
    }
...
private func openThreadIfVisible(_ title: String, root: AXUIElement) -> Bool {
    ...
    let exact = allNodes(root).first { node in
        node.role == (kAXRowRole as String) &&
            [node.value, node.description, node.title].contains(where: { $0 == title })
    }
    guard let row = exact, press(row) else { return false }
...
private func openNewTask(app: NSRunningApplication, root: AXUIElement) throws {
    ...
    let priorComposer = findComposer(allNodes(root))
    let priorWasFresh = priorComposer?.help.contains("Start a task") == true
    guard let button = allNodes(root).first(where: {
        $0.role == (kAXButtonRole as String) && $0.enabled &&
    ...
    let ready = waitUntil(timeout: 5) {
        guard let freshComposer = allNodes(root).first(where: {
            $0.role == (kAXTextAreaRole as String) && $0.valueSettable &&
                $0.help.contains("Start a task")
        }) else { return false }
        // Require an observable transition. Existing completed tasks gain the
        // fresh-task help text; an already-fresh draft must yield a new AX
        // composer element so stale request-scoped folder chips cannot survive.
        guard let priorComposer else { return true }
        return !priorWasFresh || !CFEqual(
            priorComposer.element, freshComposer.element
        )
    }
    if !ready { throw DriverError("Perplexity did not open a fresh task composer.") }
}
...
private func probeWorkspaceFolder(_ folder: String) throws -> [String: Any] {
    ...
    try openNewTask(app: app, root: root)
    defer {
        // A probe never sends a prompt. Open another blank task to discard the
        // request-scoped test grant, then restore the user's prior visible task.
        try? openNewTask(app: app, root: root)
