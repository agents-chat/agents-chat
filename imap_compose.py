...
from remote_host_safety import UnsafeRemoteHost, validate_remote_host
...
def _connect(cfg: dict) -> imaplib.IMAP4_SSL:
    try:
        validate_remote_host(cfg["imap_host"], cfg["imap_port"], purpose="IMAP")
    except UnsafeRemoteHost as exc:
        raise EmailComposeError(str(exc)) from exc
    try:
...
def smtp_send(cfg: dict, msg: EmailMessage) -> None:
    try:
        validate_remote_host(cfg["smtp_host"], cfg["smtp_port"], purpose="SMTP")
        with smtplib.SMTP_SSL(cfg["smtp_host"], cfg["smtp_port"], timeout=CONNECT_TIMEOUT) as s:
            s.login(cfg["account"], cfg["password"])
            s.send_message(msg)
    except UnsafeRemoteHost as e:
        raise EmailComposeError(str(e)) from e
    except Exception as e:
