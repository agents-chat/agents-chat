...
`mail.example.com:993`). The mailbox is ALWAYS selected read-only, so a
...
def email_search(account: str, app_password: str, query: str = "",
    ...
    try:
        validate_remote_host(imap_host, imap_port, purpose="IMAP")
    except UnsafeRemoteHost as exc:
        raise EmailSearchError(str(exc)) from exc
