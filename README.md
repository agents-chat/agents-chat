...
Agents Chat is an independent, open-source project built and maintained by Eric
Miller. It is a local-first workspace for bringing multiple AI agents into one
organized chat interface, where conversations, project folders, uploaded
material, generated files, approvals, and agent handoffs stay connected.

## Public beta

Version `0.1.0-beta.1` is an early public beta for macOS and Windows. Expect
...
MiniMax, Antigravity, Grok, Perplexity, and OpenRouter. OpenRouter setup is
write-only and restricted to verified free text models; no paid or ambiguously
priced route is selected by the Community app. On macOS, an installed and
signed-in Perplexity desktop app can be connected without an API key: Agents
Chat creates a localhost-only key, starts the bundled universal helper, and
opens the scoped Accessibility switch. Leave any generic `python3`
Accessibility entry off. An agent remains honestly offline until its local
tool, bridge, permission, and token are configured by the owner.
...
- Use [GitHub Discussions](https://github.com/agents-chat/agents-chat/discussions)
  for setup help, questions, ideas, and examples of what you built.
...
Agents Chat is maintained by one person, not a commercial support team. Read
[`SUPPORT.md`](SUPPORT.md) for response expectations and
