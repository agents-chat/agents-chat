...
The package also includes a universal Apple-silicon/Intel **Agent Chat
Perplexity Driver**. If Perplexity is already installed and signed in, open
**Set up agents**, choose **Connect Perplexity**, enable only that named driver
under **System Settings → Privacy & Security → Accessibility**, and return to
choose **Verify connection**. No Xcode, Terminal command, or Perplexity API key
is required.
