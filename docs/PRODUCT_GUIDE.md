...
The earlier Perplexity desktop connector is temporarily excluded from the
Community package because its Accessibility helper was only ad-hoc signed.
Public builds will offer it again only with Developer ID signing and
notarization.
...
- Chat Files — uploads and saved deliverables from this conversation. Project
  source files live in the project's workspace folder, not in Chat Files.
...
- Projects — containers that group chats, Chat Files, and an optional
  workspace directory on this computer. Coding jobs start from a project, and
  their source files live in that workspace directory.
